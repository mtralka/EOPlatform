...
class EOCompositeFactory:
    ...
    @staticmethod
    def _find_composite_names() -> List[str]:
    ...
    def _produce_composite(composite_dict: Dict[str, Any]) -> Composite:
        ...
        composite_type_str: str = cast(str, composite_dict.pop("type")).upper().strip()
        ...
        # composite_bands: List[str] = composite_dict.pop("bands")
        ...
        composite_docstring: str = f"""
            {composite_dict.get("name")} ({composite_dict.get("abbreviation")})
            ...
            Bands:
            {', '.join(list(cast(List[str],composite_dict["bands"])))}
